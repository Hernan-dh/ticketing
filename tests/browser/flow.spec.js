...
test('catalog, Pixi map, checkout, QR, and single admission',async({page})=>{
 ...
 await expect(page.locator('.checkout')).toContainText('45.000');await page.getByRole('button',{name:'Reservar por 5 minutos'}).click();await expect(page.locator('.checkout')).toContainText('A1');await expect(page.locator('.checkout')).toContainText('45.000');await page.getByLabel('Nombre del comprador').fill('Persona Ficticia');await page.getByLabel('Correo del comprador').fill('browser@example.com');
 ...
 await page.getByRole('button',{name:'Switch to English'}).click();await expect(page.locator('html')).toHaveAttribute('lang','en');await expect(page.getByRole('button',{name:'Brands',exact:false})).toBeVisible();await page.getByRole('button',{name:'Brands',exact:false}).click();await expect(page.getByRole('heading',{name:'New brand'})).toBeVisible();await expect(page.getByLabel('Brand name').first()).toBeVisible();await page.getByRole('button',{name:'Events',exact:false}).click();await expect(page.getByLabel('Filter by brand')).toBeVisible();await expect(page.getByRole('option',{name:'All brands'})).toBeAttached();
 await page.getByRole('button',{name:'Cambiar a español'}).click();await expect(page.locator('html')).toHaveAttribute('lang','es');await expect(page.getByRole('button',{name:'Marcas',exact:false})).toBeVisible();
 await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'Eventos',exact:true}).click();await page.screenshot({path:'test-results/catalog-mobile.png',fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);expect(errors).toEqual([]);
});
