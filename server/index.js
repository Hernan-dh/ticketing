...
import { reserve, checkout, scan, eventInput, fail, demoPaymentMethods } from './domain.js';
...
app.get('/api/events',async(_q,r)=>{if(process.env.GRAILS_URL){const events=await catalog();await store.transaction(s=>{for(const e of events)if(!s.events.some(existing=>existing.id===e.id))s.events.push(e);});}r.json(await store.transaction(s=>({events:s.events,brand:s.brand})));});
app.post('/api/events',admin,async(q,r)=>{const e=eventInput(q.body);const event=process.env.GRAILS_URL?await catalog('POST',e):e;await store.transaction(s=>s.events.push(event));r.status(201).json(event);});
app.get('/api/events/:id/seats',async(q,r)=>r.json(await store.transaction(s=>({sold:s.tickets.filter(t=>t.eventId===q.params.id).map(t=>t.seat),held:s.holds.filter(h=>h.eventId===q.params.id&&h.expiresAt>Date.now()).flatMap(h=>h.seats)}))));
app.post('/api/holds',async(q,r)=>{if(q.body.channel!=='Web'){let accepted=false;admin(q,r,()=>{accepted=true;});if(!accepted)return;}r.status(201).json(await store.transaction(s=>reserve(s,q.body)));});
app.post('/api/checkout',async(q,r)=>{if(process.env.ALLOW_DEMO_PAYMENTS!=='true')fail('El gateway de pago todavía no está configurado.',503);r.json(await store.transaction(s=>checkout(s,q.body)));});
app.post('/api/scan',admin,async(q,r)=>r.json(await store.transaction(s=>scan(s,q.body))));
app.get('/api/admin',admin,async(_q,r)=>r.json(await store.transaction(s=>({orders:s.orders,checkedIn:s.tickets.filter(t=>t.usedAt).length,tickets:s.tickets.length,integrations:s.integrations}))));
