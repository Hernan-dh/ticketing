...
async function createMysqlStore(pool) {
 ...
 return {
  ...
  checkout:body=>withState(async(state,c)=>{
   ...
   const customerId=randomUUID(),orderId=randomUUID(),methodId=randomUUID(),paymentId=randomUUID(),createdAt=new Date().toISOString(),providerReference=`demo_${randomBytes(16).toString('hex')}`,pseudonym=`Cliente ${createHmac('sha256',secrets.contact).update(email.trim().toLowerCase()).digest('hex').slice(0,12)}`;
   ...
   return {id:orderId,holdId:hold.id,eventId:event.id,total:hold.total,channel:hold.channel,email,createdAt,payment:`DEMO · ${method.label}`,tickets,paymentIntent:{id:paymentId,provider:'demo',providerReference,methodType:method.type,status:'demo_paid',amount:hold.total,currency:'ARS',createdAt}};
  }),
