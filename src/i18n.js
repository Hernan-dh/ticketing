const EN = {
  'Eventos':'Events','Ventas':'Sales','Clientes':'Customers','Control de acceso':'Access control','Integraciones':'Integrations','Marcas':'Brands','Nuevo evento':'New event',
  ...
  'Nueva marca':'New brand','Nombre de la marca':'Brand name','Color principal':'Primary color','Agregar marca':'Add brand','Guardar identidad':'Save identity','Todas las marcas':'All brands','Filtrar por marca':'Filter by brand','Conectá tu ecosistema':'Connect your ecosystem','Registrá los proveedores que necesitás conectar. Esta demo guarda la intención; no conecta servicios externos ni almacena credenciales.':'Register the providers you need. This demo records the intent; it does not connect external services or store credentials.','Tipo':'Type','Proveedor':'Provider','Nombre del proveedor':'Provider name','Agregar':'Add','Pago':'Payment','Correo':'Email','Entrega':'Delivery','Beneficios':'Benefits','Sitio de venta':'Sales site','Pendiente de conexión':'Pending connection','Entorno de demostración':'Demo environment',
  'Ver terminal del servidor':'See server terminal','Buscar eventos':'Search events','Buscar evento o lugar…':'Search event or venue…','Usá los botones de asientos debajo del plano.':'Use the seat buttons below the map.','Valentina Robles (ficticio)':'Valentina Robles (fictional)','vos@ejemplo.com':'you@example.com'
  ...
  ,'Método de pago':'Payment method','Tarjeta simulada':'Simulated card','Transferencia simulada':'Simulated bank transfer','Modo demostración: no ingreses datos de tarjeta, CBU ni comprobantes.':'Demo mode: do not enter card or bank account details or payment receipts.','Sin perfil cifrado':'No encrypted profile','Acceso rechazado:':'Access denied:','Cliente':'Customer'
  ,'Ingresá la clave de operador.':'Enter the operator key.','Demasiadas solicitudes. Esperá un minuto.':'Too many requests. Wait one minute.','Servicio no disponible. Intentá nuevamente.':'Service unavailable. Try again.','Completá nombre, lugar y fecha válida.':'Enter a valid name, venue, and date.','Elegí una marca válida.':'Choose a valid brand.','Nombre o color inválidos.':'Invalid name or color.','Marca inexistente.':'Brand not found.','Precio inválido.':'Invalid price.','El plano admite entre 1 y 20 filas y columnas.':'The map supports between 1 and 20 rows and columns.','Medio de acceso inválido.':'Invalid access method.','Evento inexistente.':'Event not found.','Canal inválido.':'Invalid channel.','Seleccioná entre 1 y 8 asientos válidos.':'Select between 1 and 8 valid seats.','Otro comprador tomó un asiento. Actualizá la selección.':'Another buyer took a seat. Refresh your selection.','La reserva venció. Volvé a elegir asientos.':'The hold expired. Choose your seats again.','Ingresá un nombre válido.':'Enter a valid name.','Ingresá un correo válido.':'Enter a valid email.','Cliente inexistente.':'Customer not found.','Entrada inválida para este evento.':'Invalid ticket for this event.','Esta entrada ya fue utilizada.':'This ticket has already been used.'
};
...
export function localize(root, lang) {
  ...
  root.querySelectorAll('[placeholder],[aria-label]').forEach(el => {
    ...
    for (const attr of ['placeholder','aria-label']) if (el.hasAttribute(attr)) {
      const values = originalAttributes.get(el),current=el.getAttribute(attr);
      if (!values[attr]) values[attr]={source:current,rendered:current};
      else if(current!==values[attr].rendered) values[attr].source=current;
      values[attr].rendered=translateValue(values[attr].source,lang);
      if(current!==values[attr].rendered) el.setAttribute(attr,values[attr].rendered);
    }
  });
}
